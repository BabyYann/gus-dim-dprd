...
// Toggle Tampilan Kata Sandi Web
function toggleWebPassword() {
  const p = document.getElementById('loginPassword');
  const icon = document.getElementById('toggleWebPasswordIcon');
  const txt = document.getElementById('toggleWebPasswordText');
  if (!p) return;
  if (p.type === 'password') {
    p.type = 'text';
    ...
    p.type = 'password';
    if (txt) txt.textContent = 'Lihat';
    if (icon) {
      icon.innerHTML = '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>';
    }
  }
}
window.toggleWebPassword = toggleWebPassword;
  ...
  function doLogin() {
    ...
    google.script.run
      .withSuccessHandler(function (res) {
        btn.disabled = false;
        btn.innerHTML = '<span>Masuk ke Sistem</span><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>';
        if (res.success) {
